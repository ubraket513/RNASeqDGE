...
for (name in names(flags)) {
  path <- Sys.which(name)
  report$tools[[name]] <- if (!nzchar(path))
    list(path = NULL) else c(list(path = unname(path)), tryCatch(run(c(path, flags[[name]]), timeout = 30, check = FALSE),
    error = function(e) list(error = conditionMessage(e))))
}
report$r_packages <- setNames(lapply(c("DESeq2", "apeglm", "BiocParallel", "pheatmap", "jsonlite"), function(name) if (requireNamespace(name,
  quietly = TRUE)) as.character(packageVersion(name)) else "MISSING"), c("DESeq2", "apeglm", "BiocParallel",
  "pheatmap", "jsonlite"))
