...
set.seed(17092026)
cd <- data.frame(condition = factor(rep(c("control", "treated"), each = 3), levels = c("control", "treated")),
  row.names = paste0("sample_", 1:6))
x <- matrix(rnbinom(6000, mu = rep(seq(40, 440, length.out = 1000), 6), size = 10), nrow = 1000, dimnames = list(paste0("gene_",
  1:1000), rownames(cd)))
...
shrunk <- lfcShrink(dds, coef = "condition_treated_vs_control", res = raw, type = "apeglm", quiet = TRUE,
  parallel = FALSE)
stopifnot(nrow(raw) == 1000, identical(rownames(raw), rownames(shrunk)), all(is.finite(shrunk$log2FoldChange)),
