...
copy_fastq_prefix<-function(source,destination,records,check_deadline=function(){}) {
 for(record in seq_len(records)) {
  ...
  assert(nchar(lines[2],type='bytes')>0L&&nchar(lines[2],type='bytes')==nchar(lines[4],type='bytes'),'FASTQ sequence/quality length mismatch')
  assert(grepl('^[ACGTNacgtn]+$',lines[2]),'invalid FASTQ sequence');qualities<-as.integer(charToRaw(lines[4]));assert(all(qualities>=33L&qualities<=126L),'invalid FASTQ quality')
  writeLines(lines,destination,sep='\n',useBytes=TRUE)
 }
}
main<-function(){
 args<-parse_cli(list(output=file.path(ROOT,'tests/output/p6-local-data'),samples=file.path(ROOT,'tests/output/p6-inputs/subset6/samples.tsv'),mapping=file.path(ROOT,'tests/output/p6-inputs/sample_run_mapping.tsv'),sources=file.path(ROOT,'config/gse80336-sources.json'),records='50000',deadline=NULL,timeout='30','ena-scheme'='https','reuse-reference-downloads'=FALSE,'read-stage'=NULL),required='deadline',flags='reuse-reference-downloads')
 records<-as.integer(args$records);timeout<-as.integer(args$timeout);assert(!is.na(records)&&records>0L&&!is.na(timeout)&&timeout>0L,'records/timeout must be positive');assert(args[['ena-scheme']] %in% c('https','http','ftp'),'invalid ENA transport')
 deadline<-as.POSIXct(args$deadline,format='%Y-%m-%dT%H:%M:%SZ',tz='UTC');assert(!is.na(deadline),'deadline must be UTC ISO8601');start<-Sys.time()
 check_deadline<-function()assert(Sys.time()<deadline,'staging deadline exceeded')
 remaining<-function(){check_deadline();max(1L,as.integer(difftime(deadline,Sys.time(),units='secs')))}
 curl_args<-function(url)c('curl','--fail','--location','--silent','--show-error','--connect-timeout',timeout,'--max-time',remaining(),url)
 out<-absolute(args$output);downloads<-file.path(out,'downloads');reads_dir<-file.path(out,'reads');dir.create(downloads,recursive=TRUE,showWarnings=FALSE);dir.create(reads_dir,showWarnings=FALSE)
 assert(!file.exists(file.path(out,'STAGED')),'output already staged')
 sources<-read_json(args$sources);assert(sources$reference$assembly==ASSEMBLY,'assembly mismatch')
 samples<-table_read(args$samples);mapping<-table_read(args$mapping);assert(nrow(samples)==6L&&!anyDuplicated(samples$sample_id),'six distinct samples required');assert(!anyDuplicated(mapping$sample_id),'duplicate sample/run mapping')
 selected<-mapping[match(samples$sample_id,mapping$sample_id),,drop=FALSE];assert(!anyNA(selected)&&all(selected$layout=='single'&selected$strandedness=='reverse'),'expected mapped reverse single-end samples')
 provenance<-list(study='GSE80336',assembly=ASSEMBLY,accession=ACCESSION,started_utc=format(start,'%Y-%m-%dT%H:%M:%SZ',tz='UTC'),deadline_utc=args$deadline,records_per_run=records,scope='restricted-reference smoke check, not a full method benchmark',sampling='first N complete FASTQ records in archive order; nonrandom prefix sampling may be biased',source_reads_full_md5_verified=FALSE,references=list(),reads=list())
 for(name in names(sources$reference$files)) {
  check_deadline();info<-sources$reference$files[[name]];path<-file.path(downloads,name)
  if(!isTRUE(args[['reuse-reference-downloads']])) {partial<-paste0(path,'.partial');run(c(curl_args(info$url),'-o',partial));assert(file.rename(partial,path),'reference publish failed')}
  actual<-unname(tools::md5sum(path));assert(!is.na(actual)&&actual==info$md5,paste('full reference MD5 mismatch:',name))
 ...
 for(row in rows_list(selected)) {
  ...
  if(is.null(old)) {
   ...
   matches<-Filter(function(x)x$run_id==row$run_id,old$reads);assert(length(matches)==1L,'missing source prefix');prior<-matches[[1]];original<-file.path(args[['read-stage']],prior$path)
   assert(sha256(original)==prior$sha256&&prior$prefix_records>=records,'source prefix hash/count mismatch');incoming<-file(original,'r');url<-prior$url
  }
 ...
 assert(exon_rows>0L&&length(genes)>0L,'no annotated exons');copy_files(args$samples,file.path(out,'samples.tsv'))
 table_write(data.frame(run_id=selected$run_id,sample_id=selected$sample_id,fastq_1=paste0('reads/',selected$run_id,'.fastq'),fastq_2='',layout='single',strandedness='reverse'),file.path(out,'runs.tsv'))
 names<-c('reference.fa','reference.gtf');suffix<-c('_genomic.fna.gz','_genomic.gtf.gz')
 table_write(data.frame(role=c('genome','annotation'),path=names,sha256=vapply(file.path(out,names),sha256,''),source=vapply(suffix,function(s)paste0(sources$reference$files[[paste0(ASSEMBLY,s)]]$url,'#',ACCESSION),''),release=ASSEMBLY),file.path(out,'references.tsv'))
 kv_write(list(version='1',design_terms='condition',alpha='0.05',filter='zero_total',shrinkage='apeglm',reference.condition='control',type.age='numeric',type.sex='categorical',type.pmi='numeric',type.rin='numeric'),file.path(out,'analysis.tsv'))
 table_write(data.frame(contrast_id='bipolar_vs_control',factor='condition',numerator='bipolar',denominator='control'),file.path(out,'contrasts.tsv'));table_write(data.frame(gene_id=sort(names(genes))),file.path(out,'genes.tsv'));table_write(do.call(rbind,genes[sort(names(genes))]),file.path(out,'annotation.tsv'))
 provenance$reference_bases<-bases;provenance$exon_rows<-exon_rows;provenance$genes<-length(genes);provenance$elapsed_seconds<-as.numeric(difftime(Sys.time(),start,units='secs'));provenance$completed_utc<-format(Sys.time(),'%Y-%m-%dT%H:%M:%SZ',tz='UTC')
 write_json(provenance,file.path(out,'provenance.json'));writeLines('restricted-reference smoke data prepared; not full-method validation',file.path(out,'STAGED'));cat('STAGED',out,length(genes),'genes\n')
}
