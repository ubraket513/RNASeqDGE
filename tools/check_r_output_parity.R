...
stopifnot(length(files) > 0L, identical(files, list.files(file.path(args[2], "contrasts"), pattern = "\\.tsv$",
  recursive = TRUE)))
for (file in files) {
  ...
  for (name in names(reference)) {
    ...
    if (!identical(is.na(a), is.na(b)))
      stop("NA mask mismatch: ", file, " / ", name)
    if (is.numeric(b)) {
      keep <- !is.na(b)
      if (any(abs(a[keep] - b[keep]) > 1e-10 + 1e-07 * abs(b[keep])))
        stop("numeric mismatch: ", file, " / ", name)
    } else if (!identical(a, b))
      stop("identity mismatch: ", file, " / ", name)
  }
}
