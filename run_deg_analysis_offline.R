...
parse_cli <- function(args) {
  ...
  for (i in seq.int(1L, length(args), by = 2L)) {
    name <- sub("^--", "", args[i])
    require_true(startsWith(args[i], "--") && name %in% allowed && !name %in% seen,
                 "unknown or duplicate option: ", args[i])
    require_true(nzchar(args[i + 1L]), "empty option: ", name)
    values[[name]] <- args[i + 1L]; seen <- c(seen, name)
  }
  require_true(all(c("counts", "samples", "analysis", "contrasts", "out") %in% seen),
               "required options: --counts --samples --analysis --contrasts --out")
  require_true(grepl("^[1-9][0-9]*$", values$workers) &&
               is.finite(as.numeric(values$workers)) && as.numeric(values$workers) <= .Machine$integer.max,
               "workers must be a positive integer")
  values$workers <- as.integer(values$workers)
  values
}
read_tsv <- function(path, schema = NULL) {
  require_true(file_test("-f", path) && file.access(path, 4) == 0,
               "input must be a readable regular file: ", path)
  ...
  lines <- withCallingHandlers(readLines(path, encoding = "UTF-8", warn = FALSE),
    warning = function(w) fail(path, ": ", conditionMessage(w)))
  require_true(length(lines) > 0L, "empty TSV: ", path)
  lines[1] <- sub("^\ufeff", "", lines[1])
  cell <- '(?:"(?:[^"\\t\\r\\n]|"")*"|[^"\\t\\r\\n]*)'
  syntax <- paste0("^", cell, "(\\t", cell, ")*$")
  require_true(all(validUTF8(lines)) && all(nzchar(lines)) &&
    !any(startsWith(lines, "#")) && !any(grepl("\ufeff", lines, fixed = TRUE)) &&
    all(grepl(syntax, lines, perl = TRUE)), "invalid TSV record or cell: ", path)
  ...
  close(connection)
  connection <- textConnection(lines)
  x <- withCallingHandlers(read.delim(connection, header = TRUE, sep = "\t", quote = '"',
    comment.char = "", colClasses = "character", na.strings = character(),
    check.names = FALSE, stringsAsFactors = FALSE, fill = FALSE, blank.lines.skip = FALSE,
    row.names = NULL), warning = function(w) fail(path, ": ", conditionMessage(w)))
  require_true(length(names(x)) > 0L && all(nzchar(names(x))) && !anyDuplicated(names(x)),
               "empty or duplicate columns: ", path)
  require_true(all(vapply(x, function(v) all(!is.na(v) & validUTF8(v)), TRUE)),
               "missing cells or invalid UTF-8: ", path)
  if (!is.null(schema)) require_true(setequal(names(x), schema), "invalid schema: ", path)
  x
}
...
validate_inputs <- function(opt) {
  ...
  require_true(!file.exists(opt$out) && !dir.exists(opt$out) && !is_symlink(opt$out),
               "output exists or aliases an existing path: ", opt$out)
  ...
  require_true(length(reserved_ids) == 0L, opt$samples, ": sample_id value(s) ",
               paste(reserved_ids, collapse = ", "), " are reserved for output identity columns")
  reserved_covariates <- intersect(names(samples), c("PC1", "PC2", "PC1_variance", "PC2_variance"))
  require_true(length(reserved_covariates) == 0L, opt$samples, ": covariate column(s) ",
               paste(reserved_covariates, collapse = ", "), " collide with reserved pca_data.tsv columns")
  ...
  require_true(settings[["version"]] == "1" && settings[["filter"]] == "zero_total" &&
    settings[["shrinkage"]] == "apeglm", "analysis requires version=1, filter=zero_total, shrinkage=apeglm")
  ...
  require_true(grepl("^[A-Za-z][A-Za-z0-9_]*(,[A-Za-z][A-Za-z0-9_]*)*$", design_text),
               "design_terms must be comma-separated declared identifiers")
  terms <- strsplit(design_text, ",", fixed = TRUE)[[1]]
  require_true(!anyDuplicated(terms) && all(terms %in% setdiff(names(samples), "sample_id")),
               "duplicate or absent design covariate")
  types <- c(condition = "categorical"); refs <- character()
  ...
  for (term in setdiff(names(samples), "sample_id")) {
    ...
    if (types[[term]] == "numeric") samples[[term]] <- decimal(samples[[term]], paste("numeric covariate", term))
    else require_true(all(nzchar(samples[[term]])), "empty categorical covariate: ", term)
  }
  ...
  for (i in seq_len(nrow(contrasts))) {
    row <- contrasts[i, ]; term <- row$factor
    ...
    if (term %in% names(refs)) require_true(refs[[term]] == row$denominator,
                                           "explicit reference disagrees with contrast denominator")
  }
  ...
  require_true(names(input)[1] == "gene_id" && identical(names(input)[-1], samples$sample_id),
               "count/sample identity and order must match exactly")
  require_true(nrow(input) > 0L && all(nzchar(input$gene_id)) && !anyDuplicated(input$gene_id),
               "empty or duplicate gene ID")
  count_text <- as.matrix(input[-1])
  values <- suppressWarnings(as.numeric(count_text))
  require_true(all(grepl("^[0-9]+$", count_text)) && all(is.finite(values)) &&
               all(values <= .Machine$integer.max), "counts must be decimal integers in [0, 2147483647]")
  counts <- matrix(as.integer(values), nrow(input), ncol(input) - 1L,
  ...
  if (!is.null(opt$annotation)) {
    ...
    require_true(all(nzchar(annotation$gene_id)) && !anyDuplicated(annotation$gene_id) &&
                 all(annotation$gene_id %in% input$gene_id), "annotation has duplicate, empty or unknown gene IDs")
  }
  ...
  for (i in seq_len(nrow(contrasts))) {
    contrast <- contrasts[i, ]; cd <- samples; rownames(cd) <- cd$sample_id; cd$sample_id <- NULL
    for (term in names(types)[types == "categorical"]) {
      observed <- sort(unique(cd[[term]]), method = "radix")
      reference <- if (term == contrast$factor) contrast$denominator else if (term %in% names(refs)) refs[[term]] else observed[1]
    ...
    require_true(!"sample_id" %in% colnames(mm), opt$samples,
                 ": model column 'sample_id' collides with reserved model_matrix.tsv identity column")
  ...
  list(out = out, counts = counts, samples = samples, annotation = annotation, alpha = alpha,
       contrasts = contrasts, models = models, formula = formula, genes_input = nrow(input),
       genes_zero_total = sum(!keep))
}
...
write_matrix <- function(x, name, path) {
  ids <- data.frame(id = as.character(rownames(x)), stringsAsFactors = FALSE); names(ids) <- name
...
run_contrast <- function(data, i, opt, stage, bp) {
  ...
  result_table <- function(result) {
    ...
    if (!is.null(data$annotation)) {
      ann <- data$annotation[match(tab$gene_id, data$annotation$gene_id), c("gene_symbol", "biotype", "chromosome"), drop = FALSE]
      for (column in names(ann)) ann[[column]][!is.na(ann[[column]]) & ann[[column]] == ""] <- NA_character_
      rownames(ann) <- NULL; tab <- cbind(tab, ann)
    }
    tab
  }
  ...
  stats <- c(contrast_id = contrast$contrast_id, factor = contrast$factor,
    ...
    alpha = data$alpha, genes_input = data$genes_input, genes_zero_total = data$genes_zero_total,
    genes_retained = nrow(raw), significant = sum(significant),
  ...
  variance <- if (total_variance > 0) pc$sdev^2 / total_variance else rep(0, length(pc$sdev))
  pca <- data.frame(sample_id = rownames(pc$x), PC1 = pc$x[, 1], PC2 = pc2,
  ...
  plot_png(file.path(root, "volcano.png"), function() {
    ...
    if (!any(finite)) {plot.new(); text(0.5, 0.5, "No finite adjusted p-values")} else {
      plot(volcano$log2FoldChange[finite], volcano$neg_log10_padj[finite], pch = 16, cex = 0.5,
           col = ifelse(significant[finite], "firebrick", "grey60"), main = title,
           xlab = "apeglm log2 fold change", ylab = "-log10 adjusted p-value")
      abline(h = -log10(data$alpha), lty = 2)
    }
  })
  plot_png(file.path(root, "pca.png"), function() {
    plot(pca$PC1, pca$PC2, pch = 19, col = as.integer(model$data[[contrast$factor]]),
         xlab = sprintf("PC1 (%.1f%%)", 100 * pca$PC1_variance[1]),
         ylab = sprintf("PC2 (%.1f%%)", 100 * pca$PC2_variance[1]), main = "VST sample PCA")
    text(pca$PC1, pca$PC2, labels = pca$sample_id, pos = 3, cex = 0.7)
  })
  plot_png(file.path(root, "sample_distances.png"), function()
    pheatmap::pheatmap(distance, main = "Euclidean distances of VST samples", silent = FALSE))
  plot_png(file.path(root, "heatmap.png"), function() {
    if (nrow(heat) == 0L) {plot.new(); text(0.5, 0.5, "No significant genes (adjusted p-value < alpha)")}
    else pheatmap::pheatmap(heat, scale = "none", cluster_rows = nrow(heat) > 1L,
...
tryCatch(main(), error = function(e) {
  message("ERROR: ", conditionMessage(e)); quit(save = "no", status = 1L)
})
