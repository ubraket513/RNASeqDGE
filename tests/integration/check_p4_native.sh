...
trap 'rm -rf "$tmp"' EXIT
mkdir "$tmp/bin"
cat > "$tmp/bin/hisat2-build" <<'STUB'
#!/bin/sh
if [ "$1" = --version ]; then echo 'hisat2-build version 2.2.1'; exit; fi
printf '%s\n' "$@" > "$P4_ARGS"
exit 17
STUB
...
test ! -e "$tmp/output"
test -f "$P4_ARGS"
test "$(sed -n 1p "$P4_ARGS")" = -p
test "$(sed -n 2p "$P4_ARGS")" = 1
grep -q '17' "$tmp/log"
...
cat > "$tmp/bin/hisat2-build" <<'STUB'
#!/bin/sh
if [ "$1" = --version ]; then echo "hisat2-build version $P4_STUB_VERSION"; exit; fi
printf work > "$P4_ARGS"
for prefix; do :; done
for i in 1 2 3 4 5 6 7 8; do printf x > "$prefix.$i.ht2"; done
STUB
