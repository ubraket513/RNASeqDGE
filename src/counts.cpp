...
void readable_regular(const fs::path &path, const std::string &source, std::size_t record,
...
fs::path data_path(const Table &table, std::size_t record, const std::string &field,
                   const std::string &value) {
    if (value.empty())
        fail(table, record, field, "path is empty");
...
GeneUniverse read_genes(const fs::path &path) {
    ...
    for (std::size_t row = 0; row < genes.rows.size(); ++row) {
        ...
        if (id.empty())
            fail(genes, row + 2, "gene_id", "gene ID is empty");
...
void reject_output_alias(const fs::path &output, const fs::path &input) {
    ...
    if (!fs::exists(input, input_error) || input_error)
        return;
    std::error_code output_error;
    if (!fs::exists(output, output_error) || output_error)
        return;
...
void reject_output_aliases(const fs::path &output, const std::vector<fs::path> &inputs) {
    for (const auto &input : inputs)
        reject_output_alias(output, input);
}

std::ifstream open_regular(const fs::path &path, const std::string &source, std::size_t record,
                           const std::string &field) {
    readable_regular(path, source, record, field);
    std::ifstream input(path, std::ios::binary);
    if (!input)
        fail(source, record, field, "cannot open input");
    return input;
}

void require_header(const std::vector<std::string> &actual,
                    const std::vector<std::string> &expected, const std::string &source) {
    if (actual != expected)
        fail(source, 1, "1", "unexpected header");
}

std::size_t unique_header_column(const std::vector<std::string> &header,
                                 const std::string &selected, const std::string &source) {
    ...
    if (found == header.end())
        fail(source, 1, selected, "selected column missing");
    return static_cast<std::size_t>(found - header.begin());
}

void add_count(const std::vector<std::string> &row, std::size_t gene_column,
               std::size_t count_column, const std::string &count_name, const std::string &source,
               std::size_t record, const GeneUniverse &genes, std::vector<bool> &seen,
    ...
    if (seen[gene])
        fail(source, record, "gene_id", "duplicate gene ID");
    seen[gene] = true;
    const auto value =
        count_value(row[count_column],
                    source + ": record " + std::to_string(record) + ", column " + count_name);
    const auto offset = gene * (totals.size() / genes.ids.size()) + sample;
    try {
        totals[offset] = checked_add(totals[offset], value);
    } catch (const std::overflow_error &) {
        throw std::overflow_error(source + ": record " + std::to_string(record) + ", column " +
                                  count_name + ": count sum exceeds uint64 range");
    }
}

void require_complete_universe(const std::vector<bool> &seen, const GeneUniverse &genes,
...
void read_native_counts(const fs::path &path, const GeneUniverse &genes,
    ...
    if (!reader.next(row))
        fail(path.string(), 1, "1", "empty file");
    require_header(row, {"gene_id", "count"}, path.string());
    std::vector<bool> seen(genes.ids.size());
    while (reader.next(row))
        add_count(row, 0, 1, "count", path.string(), reader.record(), genes, seen, totals, sample);
...
void validate_featurecounts_summary(const fs::path &path, const std::string &selected,
    ...
    std::vector<std::string> row;
    if (!reader.next(row))
        fail(path.string(), 1, "1", "empty summary");
    ...
    while (reader.next(row)) {
        ...
        for (std::size_t i = 1; i < row.size(); ++i)
            count_value(row[i], path.string() + ": record " + std::to_string(reader.record()) +
                                    ", column " + std::to_string(i + 1));
    }
    if (!assigned)
        fail(path.string(), 1, "Status", "Assigned status missing");
}

void read_featurecounts(const fs::path &path, const std::string &selected,
                        const GeneUniverse &genes, std::vector<std::uint64_t> &totals,
                        std::size_t sample, const std::string &manifest_source,
                        std::size_t manifest_record) {
    auto input = open_regular(path, manifest_source, manifest_record, "path");
    ...
    if (row.size() < prefix.size() + 1 || !std::equal(prefix.begin(), prefix.end(), row.begin()))
    ...
    while (reader.next(row)) {
        for (std::size_t i = prefix.size(); i < row.size(); ++i)
            count_value(row[i], path.string() + ": record " + std::to_string(reader.record()) +
                                    ", column " + std::to_string(i + 1));
        add_count(row, 0, selected_column, selected, path.string(), reader.record(), genes, seen,
                  totals, sample);
    }
    require_complete_universe(seen, genes, path.string());
    validate_featurecounts_summary(fs::path(path.string() + ".summary"), selected, manifest_source,
                                   manifest_record);
}
...
class TemporaryOutput {
  public:
    explicit TemporaryOutput(const fs::path &output) {
        ...
        if (descriptor == -1)
            throw std::runtime_error("cannot create temporary output beside " + output.string() +
                                     ": " + std::strerror(errno));
        path_ = buffer.data();
...
void write_counts(const fs::path &output, const GeneUniverse &genes,
    ...
    for (const auto &sample : samples)
        temporary.write("\t" + output_field(sample));
    temporary.write("\n");
...
void validate_counts_for_samples(const fs::path &samples_path, const fs::path &counts_path) {
    ...
    for (const auto &row : samples.rows)
        expected.push_back(row[sample_id]);
    if (counts.header != expected)
        fail(counts, 1, "gene_id", "count columns must exactly match sample manifest order");
}

void validate_annotation(const fs::path &genes_path, const fs::path &annotation_path) {
...
void merge_counts(const fs::path &samples_path, const fs::path &runs_path,
                  const fs::path &genes_path, const fs::path &inputs_path, const fs::path &output,
    ...
    for (std::size_t row = 0; row < inputs.rows.size(); ++row) {
        ...
        count_inputs.push_back(
            {values[format], resolved, values[selected_column], mapped->second, record});
    }
    ...
    for (const auto &source : count_inputs) {
        if (source.format == "tsv")
            read_native_counts(source.path, genes, totals, source.sample, inputs.source,
                               source.record);
        else if (source.format == "legacy")
            read_legacy_counts(source.path, genes, totals, source.sample, inputs.source,
                               source.record);
        else
            read_featurecounts(source.path, source.selected, genes, totals, source.sample,
