...
void run_jobs(std::size_t count, std::size_t concurrency,
              const std::function<void(std::size_t)> &action) {
    if (!concurrency)
        throw std::runtime_error("job concurrency must be positive");
    Signals signals;
    Subreaper subreaper;
    ...
    try {
        while (next < count || !active.empty()) {
            if (interrupted)
                throw ProcessError("local jobs interrupted", 128 + interrupted);
            ...
            for (auto it = active.begin(); it != active.end();) {
                ...
                if (result < 0)
                    throw std::runtime_error("worker waitpid failed");
                it = active.erase(it);
                const int code = WIFEXITED(status) ? WEXITSTATUS(status) : 128 + WTERMSIG(status);
                if (code)
                    throw ProcessError(
                        "local alignment worker failed with exit " + std::to_string(code), code);
            }
            if (!active.empty()) {
                const timespec delay{0, 10000000};
                nanosleep(&delay, nullptr);
...
void run_process(const std::vector<std::string> &argv, const std::filesystem::path &log,
    ...
    if (argv.empty() || argv[0].empty() || argv[0][0] != '/')
        throw std::runtime_error("executable must be absolute");
    std::vector<char *> arguments;
    for (const auto &argument : argv) {
        if (argument.find('\0') != std::string::npos)
            throw std::runtime_error("NUL in argument");
    ...
    for (char **entry = environ; *entry; ++entry)
        if (!std::string_view(*entry).starts_with("PATH="))
    ...
    if (fd < 0)
        throw std::runtime_error("cannot open process log: " + log.string());
    if (fd < 3) {
    ...
    if (fd < 0)
        throw std::runtime_error("cannot duplicate process log");
    int output_fd = fd;
    if (!stdout_log.empty()) {
        output_fd = open(stdout_log.c_str(), O_WRONLY | O_CREAT | O_TRUNC | O_CLOEXEC, 0600);
        if (output_fd >= 0 && output_fd < 3) {
            int copy = fcntl(output_fd, F_DUPFD_CLOEXEC, 3);
            close(output_fd);
            output_fd = copy;
        }
        if (output_fd < 0) {
            close(fd);
            throw std::runtime_error("cannot open stdout log: " + stdout_log.string());
        }
    }
    Signals signals;
    pid_t child = fork();
    ...
    if (child == 0) {
        ...
        signal(SIGINT, SIG_DFL);
        signal(SIGTERM, SIG_DFL);
        if (dup2(output_fd, STDOUT_FILENO) < 0 || dup2(fd, STDERR_FILENO) < 0)
            _exit(126);
        if (output_fd != fd)
            close(output_fd);
        close(fd);
    ...
    int code = interrupted         ? 128 + interrupted
               : WIFEXITED(status) ? WEXITSTATUS(status)
                                   : 128 + WTERMSIG(status);
    if (code)
        throw ProcessError(
            argv[0] + " failed with exit " + std::to_string(code) + "; log: " + log.string(), code);
}
} // namespace rnaseq
