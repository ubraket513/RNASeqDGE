...
[[noreturn]] void fail(const Table& table, std::size_t record,
    ...
    throw std::runtime_error(table.source + ": record " + std::to_string(record) +
...
fs::path data_path(const Table& table, std::size_t record,
    ...
    if (value.empty()) fail(table, record, column_name, "path is empty");
    const fs::path supplied(value);
    return fs::path(table.source).parent_path() / supplied;
}

void readable_regular(const Table& table, std::size_t record,
                      const std::string& column_name, const fs::path& path) {
    std::error_code error;
    if (!fs::is_regular_file(path, error) || error)
        fail(table, record, column_name, "path does not name a regular readable file");
    std::ifstream input(path, std::ios::binary);
    if (!input) fail(table, record, column_name, "path does not name a regular readable file");
}
...
void validate_run_mapping(const Table& samples, const Table& runs, bool check_fastq_files) {
    ...
    if (runs.rows.empty()) fail(runs, 2, "run_id", "no runs");
    ...
    for (std::size_t row = 0; row < runs.rows.size(); ++row) {
        ...
        const auto [existing, inserted] = sample_library.emplace(
            values[run_sample], std::pair{values[layout], values[strand]});
        ...
        if (check_fastq_files) readable_regular(runs, record, "fastq_1", first);
        if (values[layout] == "single") {
            ...
            if (error) fail(runs, record, "fastq_2", "cannot compare paired FASTQ files");
        }
    }
    for (const auto& [id, record] : sample_records)
        if (!covered.contains(id)) fail(samples, record, "sample_id", "sample has no run");
}
...
void validate_bundle(const fs::path& samples_path, const fs::path& runs_path,
    ...
    for (std::size_t row = 0; row < references.rows.size(); ++row) {
        ...
        if (values[source].empty()) fail(references, record, "source", "required cell empty");
        if (values[release].empty()) fail(references, record, "release", "required cell empty");
    ...
    if (!roles.contains("genome")) fail(references, 1, "role", "genome role missing");
    if (!roles.contains("annotation")) fail(references, 1, "role", "annotation role missing");
    ...
    for (std::size_t row = 0; row < analysis.rows.size(); ++row) {
        ...
        if (name.empty() || !settings.emplace(name, Setting{analysis.rows[row][setting_value], record}).second)
    ...
    const auto require = [&](const std::string& name) -> const Setting& {
        ...
        if (found == settings.end()) fail(analysis, 1, name, "required setting missing");
        return found->second;
    };
    ...
    if (!decimal(require("alpha").value, alpha) || !(alpha > 0 && alpha < 1))
        fail(analysis, require("alpha").record, "value", "alpha must be a finite decimal between 0 and 1");
    const auto design = split_design(analysis, require("design_terms").record,
                                     require("design_terms").value);
    ...
    for (const auto& [name, setting] : settings) {
        if (name == "version" || name == "design_terms" || name == "alpha" ||
            name == "filter" || name == "shrinkage") continue;
    ...
    for (std::size_t i = 0; i < samples.header.size(); ++i) {
        ...
        for (std::size_t row = 0; row < samples.rows.size(); ++row) {
            ...
            if (types.at(name) == "categorical") {
                if (value.empty()) fail(samples, row + 2, name, "categorical value is empty");
            } else {
                double parsed = 0;
    ...
    for (const auto& term : design) {
        if (term == "sample_id" || !sample_columns.contains(term))
            fail(analysis, require("design_terms").record, "value", "design term is not a sample covariate");
    ...
    for (const auto& [field, setting] : references_by_column) {
        ...
        const bool observed = std::any_of(samples.rows.begin(), samples.rows.end(),
            [&](const auto& row) { return row[index] == setting.value; });
        if (!observed) fail(analysis, setting.record, "value", "reference level is not observed");
    }

    fixed_schema(contrasts, {"contrast_id", "factor", "numerator", "denominator"});
    if (contrasts.rows.empty()) fail(contrasts, 2, "contrast_id", "no contrasts");
    const auto contrast_id = column(contrasts, "contrast_id"), factor = column(contrasts, "factor");
    const auto numerator = column(contrasts, "numerator"), denominator = column(contrasts, "denominator");
