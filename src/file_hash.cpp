...
std::string sha256_file(const std::filesystem::path &path) {
    ...
    if (opened.value < 0)
        fail(std::string("cannot open input: ") + std::strerror(errno));
    struct stat info{};
    ...
    if (input.value < 0)
        fail("cannot duplicate input descriptor");
    int ends[2];
    if (pipe(ends) != 0)
        fail("cannot create checksum pipe");
    ...
    if (output.value < 0 || writer.value < 0)
        fail("cannot duplicate checksum pipe");
    close(raw_read.value);
    raw_read.value = -1;
    close(raw_write.value);
    raw_write.value = -1;
    ...
    if (child < 0)
        fail("cannot fork checksum subprocess");
    if (child == 0) {
        ...
        if (prctl(PR_SET_PDEATHSIG, SIGKILL) != 0 || getppid() != parent)
            _exit(125);
        if (dup2(input.value, STDIN_FILENO) < 0 || dup2(writer.value, STDOUT_FILENO) < 0)
            _exit(126);
    ...
    if (read_failed || size == sizeof(result))
        kill(child, SIGKILL);
    int status = 0;
    pid_t waited;
    do {
