...
int main(int argc, char** argv) {
    if (argc == 2 && std::string(argv[1]) == "--help") {
        std::cout << "Usage: rnaseq workflow-plan|workflow-local|workflow-submit CONFIG\n"
                     ...
                     "       rnaseq index|align-count --backend star|hisat2 --bin-dir DIR --fasta FILE --gtf FILE --output DIR [--threads N]\n"
                     "       index: [--star-sa-bases N --star-chr-bits N] (genome-only policy)\n"
                     "       align-count: --index DIR --reads1 FILE [--reads2 FILE] --layout single|paired --strandedness unstranded|forward|reverse\n";
        return (std::cout << std::flush) ? 0 : 1;
    }
    if (argc < 2) { std::cerr << "Use rnaseq --help for usage\n"; return 2; }
    try {
        const std::string command = argv[1];
        if (command.starts_with("workflow-")) { rnaseq::workflow_command(argc, argv); return 0; }
        ...
        if (command == "merge-counts") {
            if (argc != 7 && argc != 8) { std::cerr << "Use rnaseq --help for usage\n"; return 2; }
            rnaseq::merge_counts(argv[2], argv[3], argv[4], argv[5], argv[6], argc == 8 ? argv[7] : "");
            std::cout << "OK: counts merged\n";
            return (std::cout << std::flush) ? 0 : 1;
        }
        if (command == "validate-counts-for-samples" || command == "validate-annotation") {
            if (argc != 4) { std::cerr << "Use rnaseq --help for usage\n"; return 2; }
        ...
        if (command == "validate-bundle") {
            if (argc != 7) { std::cerr << "Use rnaseq --help for usage\n"; return 2; }
